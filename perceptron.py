...
class PerceptronClassifier(object):
    ...
    def train(self, input_train_data, label_train_data, input_val_data, label_val_data, iterations, callback=None):
		...
		#Q1: Question 1
                ...
                resultlabel = None 
                maxval= -float('inf')
                inputvector = input_train_data[i]
                for label in self.legal_labels:
		    # score = multiplication of two vectors: fweight[label] and inputvector
		    # Both vectors use Counter data structure and thus can use Counter multiplication
                    score = self.weights[label]*inputvector
                    if score > maxval:
                        maxval = score
                        resultlabel = label
                if resultlabel != label_train_data[i]: #wrong classification
                    self.weights[label_train_data[i]] += inputvector
                    self.weights[resultlabel] -= inputvector
...
#        util.raiseNotDefined()
	#Q3: Question 3
	weightFeaturePairs = []
        fweights = self.weights[label]

        for feature, value in fweights.iteritems():
            weightFeaturePairs.append((value,feature))
        weightFeaturePairs.sort()
        best100Features = [feature for val, feature in weightFeaturePairs[-100:]]
