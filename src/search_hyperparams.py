...
def search_hyperparams(train_data, train_labels, val_data, val_labels,
	...
	#Q8: Question 8
	# We will iteratate through different choices and find the one leading the highest accuracy
	solver=solvers.MinibatchStochasticGradientDescentSolver(learning_rate, iterations, batch_size, momentum) 

	#train this classifer using the above solver
	solver.solve(train_data, train_labels, val_data,val_labels, model)

	#compute accuracy using the validation data
	rate= model.accuracy(val_data,val_labels)
	if  rate> bestRate:
		bestRate=rate
