...
class GradientDescentSolver(Solver):
	...
	#param_vars[0]  -= self.learning_rate*grad_tensors[0]
	#param_vars[1]  -= self.learning_rate*grad_tensors[1]
	

	for i in range(len( param_vars)):
		oldweight=param_vars[i]
		newweight=oldweight - self.learning_rate*grad_tensors[i]
...
class StochasticGradientDescentSolver(GradientDescentSolver):
	...
	trainGen=tfu.MinibatchIndefinitelyGenerator(train_data,1, self.shuffle)
	validateGen=tfu.MinibatchIndefinitelyGenerator(val_data,1, self.shuffle)
